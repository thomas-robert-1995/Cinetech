...
function displayItems(items) {
  ...
  items.forEach(item => {
    ...
    div.innerHTML = `<img src="${posterPath}" alt="${item.title || item.name}" style="width: 100%; cursor:pointer" onclick="window.location.href='detail.html?id=${item.id}&type=${item.title ? 'movie' : 'tv'}'" />`;
